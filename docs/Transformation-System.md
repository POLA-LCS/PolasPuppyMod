...
A player wearing a puppy set can turn into a dog and back with the Quick Mount key, as long as no mount is equipped, or with the dedicated Dog Transformation key (default T), which also works while a mount is equipped. The dog has its own animations, emotes and a shorter, wider hitbox. The breed is chosen in the client config and shared with other players, so everyone sees the same dog.

```mermaid
flowchart TB
    subgraph Input[Input]
        MountKey[Quick Mount key]
        TransformKey[Dog Transformation key]
    ...
    TransformKey -- "transform" --> PuppySet
    PuppySet --> NoMount
    NoMount --> Form
    MountKey -- "turn back" --> Headroom
    TransformKey -- "turn back" --> Headroom
...
- **Transformation** is toggled with the Quick Mount key, which defaults to R, or with the dedicated Dog Transformation key, which defaults to T. With any puppy set and no mount equipped, Quick Mount turns the player into a dog; pressing it again turns back. With a mount equipped, Quick Mount keeps its normal mount behavior, while the dedicated key still toggles the transformation.
...
2. **Player presses Quick Mount or the Dog Transformation key** - the player turns into a dog. A puff of dust plays, and the breed is copied from the player's client config.
...
7. **Player presses Quick Mount or the Dog Transformation key again** - with about 2.7 tiles of headroom, the player returns to full size; otherwise the dog stays. The full-size hitbox returns on the next successful attempt.
