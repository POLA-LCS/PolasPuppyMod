...
The Puppy equipment system adds one more attached effect: a selected matching Reinforced Ears/Tail pair grants defense and reduced incoming knockback to both the Puppy and the Owner while the leash connection is valid.
...
flowchart TB
    subgraph PuppySide[Puppy Player - IsPuppy]
        ...
        PairBonus[Attached Puppy pair effect]
    ...
    Chained -- "valid attachment" --> PairBonus
...
- An **attached Puppy pair effect** is separate from collar and leash effects. Only the selected Ears and selected matching Tail can provide it. While attached, the Reinforced pair grants both sides additional defense by placement state - 2 (Costume), 3 (Furry), 4 (Therian) - and reduces incoming knockback to 0.8 when both selected pieces are functional (Therian), otherwise to 0.9. It is not active while the pair is merely equipped but unattached.
...
6. **While leashed, the selected pair affects both sides** - the attached-pair service checks the puppy's resolved Ears/Tail pair and applies the Reinforced defense for its placement state - 2 (Costume), 3 (Furry), 4 (Therian) - to the Puppy and Owner, and reduces incoming knockback to 0.8 when both selected pieces are functional, otherwise to 0.9, for the duration of the valid connection.
7. **Detachment and cleanup** - detaching, moving out of range, death, removing the collar, or losing puppy status invalidates the link, clears the leash state, and removes the ongoing effects on both sides.
