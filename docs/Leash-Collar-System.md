...
- An **attached Puppy pair effect** is separate from collar and leash effects. Only the selected Ears and selected matching Tail can provide it. The Reinforced pair gives both sides `+2` defense and 20% reduced incoming knockback when both selected pieces are functional; if either selected piece is vanity, the effect is half strength (`+1` defense and 10% reduced incoming knockback). It is not active while the pair is merely equipped but unattached.
...
6. **While leashed, the selected pair affects both sides** - the attached-pair service checks the puppy's resolved Ears/Tail pair and applies the Reinforced defense and incoming knockback effect to the Puppy and Owner for the duration of the valid connection.
