...
4. **Puppy loop runs** - the puppy class applies equipment defense, movement, pick-speed, jump and outgoing knockback effects, appends active set text through `Player.setBonus`, decrements the shared bark cooldown, listens for clicker signals, applies the Good Puppy buff to any player within range of a clicker, and counts down the pet wag timer. Barks and hurt sounds go through the network-aware path when the game is in multiplayer.
